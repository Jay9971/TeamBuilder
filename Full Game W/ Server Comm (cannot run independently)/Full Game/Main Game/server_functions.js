...
async function getGameState() {
  try {
      ...
      for (const key in response) {
        ...
        if (!isNaN(key)) {
          server_occupied_list.push(response[key]);
        } else if (key === "gameStatus") {
          // if the game is over, request the lobby page
          if (response[key] === "1") {
      ...
      //updates all game squares with the new images
...
function leaveGame() {
  try {
      const response = post("leave game", {
        ID: userID,
        leaveGameStatus: "1"
      }, 'application/json');
...
function sendMySquares() {
  ...
  try {
    post("listening for square data", {
      ID: userID,
